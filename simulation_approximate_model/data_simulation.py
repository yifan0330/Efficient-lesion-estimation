...
class simulated_data(object):
    ...
    def generate_data(self, covariate, lesion_size_range):
        # covariate effect 
        Y, Z, total_intensity_func = dict(), dict(), dict()
        for i in range(self.n_group):
            if covariate:
                Z_i = np.arange(start=0, stop=1, step=1/self.n_subject[i], dtype=np.float64)
                Z_i = Z_i.reshape((self.n_subject[i], -1))
                # Z_i -= np.mean(Z_i, axis=0) # standardization 
                # Z_i /= np.std(Z_i, axis=0)
            else:
                Z_i = np.zeros((self.n_subject[i], 1))
            Z[self.group_names[i]] = Z_i
        # other spatial effect shared by subjects
        seed = 0
        for k in range(self.n_group):
            Y_group_k, total_intensity_func_k = list(), list()
            group_name = self.group_names[k]
            for s in range(self.n_subject[k]):
                np.random.seed(seed)
                # combine background intensity and spatially varying covariate intensity
                sum_intensity_func_s = self.background_intensity_func[group_name] + Z[group_name][s]*self.covariate_intensity_func[group_name]
                sum_intensity_func_s = np.clip(sum_intensity_func_s, a_min=0.001, a_max=1)
                ...
                # Store data
                Y_group_k.append(Y_s)
                total_intensity_func_k.append(total_intensity_func_s)
                seed += 1
                # # 1D Visualisation
                ...
                # plt.ylabel('Y-axis')
                # plt.savefig("test.png")
            Y_group_k = np.concatenate(Y_group_k, axis=0)
            total_intensity_func_k = np.concatenate(total_intensity_func_k, axis=0)
            ...
            # Save to dictionary
            Y[group_name] = Y_group_k
            total_intensity_func[group_name] = total_intensity_func_k

        return total_intensity_func, Y, Z
