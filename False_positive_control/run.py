...
def _build_simulated_brain_filenames(base_path, space_dim, model, filename_components, optimization_params, args):
    ...
    filenames = {
        "data_filename": f"{base_path}/data/{space_dim}/data_Simulation/data{filename_components['dset']}_random_seed_{args.random_seed}.npz",
        ...
        "XTWX_filename": f"{base_path}/results/{space_dim}/GRF_{args.n_subject}/{model_params}/{space_dim}_XTWX{filename_components['dset']}{filename_components['model']}{filename_components['poly']}_random_seed_{args.random_seed}.npz",
        ...
        "fig_filename": f"{base_path}/figures/PP-plots/{space_dim}/{space_dim}_PP_plot_{args.inference_method}{common_suffix}_random_seed_{args.random_seed}.png",
    }
...
if args.run_regression:
    ...
    if not "data" in locals():
        if simulated_dset:
            load_data = np.load(data_filename, allow_pickle=True)
        else:
            load_data = np.load(masked_data_filename, allow_pickle=True)
        data = {}
        for group_name in group_names:
            data[group_name] = load_data[group_name].item()
            # add cubic terms to Z
            data[group_name]["Z"] = preprocess_Z(simulated_dset, data[group_name]["Z"], polynomial_order)
    # # smoothing
    ...
    if args.full_model:
        ...
        if not os.path.exists(results_filename):
            ...
            plot_brain(np.sqrt(P_mean[list(P_mean.keys())[0]]), smooth_lesion_mask, vmax=None, threshold=0, output_filename=filenames_dict['lesion_estimation_map_filename']) if smooth_lesion_mask is not None else plot_brain(np.sqrt(P_mean[list(P_mean.keys())[0]]), brain_mask, output_filename=filenames_dict['lesion_estimation_map_filename'])
    # if args.full_model:
...
if args.run_inference:
    ...
    if args.full_model:
        ...
        if simulated_dset:
            ...
            XTWX_filename=XTWX_filename, inference_filename=inference_filename,
