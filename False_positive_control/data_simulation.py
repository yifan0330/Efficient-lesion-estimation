...
class GRF_simulated_data(object):
    ...
    def process_data(self, mask_path, low=45.12, high=80.65, 
        ...
        for i in range(self.n_group):
            ...
            combined_mask_3d = (mask_data != 0) & (empir_prob != 0)  # combined mask in 3D
            combined_mask_in_brain = combined_mask_3d[mask_data]       # combined mask in flattened brain space
            for j in range(n):
                if j % 100 == 0:
                    print(f"subject {j}/{n}")
                grf_seed = self.combine_seeds_int(random_seed, j)
                grf = self.simulate_grf_gaussian(mask_shape, var=1, scale=1.5, random_seed=grf_seed)
                # norm.cdf(grf + xbeta) >= 0.5  ⟺  grf + xbeta >= 0
                y_bin = ((grf + xbeta) >= 0).astype(np.float32)
                y_bin = y_bin[mask_data]
                y_bin[~combined_mask_in_brain] = 0.0
                Y_i[j, :] = y_bin
