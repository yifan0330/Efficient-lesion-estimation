...
def preprocess_Z(simulated_dset, Z, polynomial_order):
    if simulated_dset:
        ...
        covariate_col -= np.mean(covariate_col, axis=0)
        # Z = covariate_col
...
def compute_gradient(Z, 
    ...
    if marginal_dist == "Poisson":
        ...
        # X^T WY where W is a diagonal matrix with W_ii = \mu_i / (1 + \alpha*\mu_i)
        mu_tmp = compute_mu(Z, B, beta, mode=mode, block_size=block_size).reshape((_M, _N))
        G = Y / (1 + alpha*mu_tmp) @ B  # [_M, _P]
        G = Z.T @ G # [_R, _P]
        XTY = G.reshape((_R * _P, 1)) # [_R * _P, 1]
    if mode == "approximate":
        ...
        if marginal_dist == "Poisson":
            ...
            # Gradient: U(\beta) = X^T W (Y - \mu), where W is a diagonal matrix with W_ii = \mu_i / (1 + \alpha*\mu_i)
            XTmu = kronecker_vector_product(Z.T, B.T, mu / (1 + alpha*mu), use_dask=True, block_size=block_size)
        with ProgressBar():
            XTmu = XTmu.compute()
    else:
...
def log_nb_likelihood(lam, Y, alpha, use_dask=False, block_size=1000):
    """Compute the log Negative Binomial likelihood.
    Args:
        lam: Array of shape [n_samples, n_features]
        Y: Array of shape [n_samples, n_features]
    ...
    if use_dask:
        ...
        if not isinstance(Y, da.Array):
            Y = da.from_array(Y, chunks=(block_size,))
        lam, Y = lam.ravel(), Y.ravel()
        return (da.multiply(Y, da.log(lam)) - da.multiply(Y + 1/alpha, da.log(1 + alpha*lam)) + (1/alpha)*da.log(1 + alpha*lam)).mean()
    return (Y.multiply(np.log(lam)) - (Y + 1/alpha).multiply(np.log(1 + alpha*lam)) + (1/alpha)*np.log(1 + alpha*lam)).mean()
...
def compute_log_nb_nll(Z, B, beta, Y, alpha, mode="exact", block_size=1000):
    _M, _R = Z.shape
    _N, _P = B.shape
    if mode == "exact":
        mu = np.exp(kronecker_vector_product(Z, B, beta))
        nll = -log_nb_likelihood(mu, Y.reshape(-1, 1), alpha)
    elif mode == "dask":
        if not isinstance(Y, da.Array):
            Y = da.from_array(Y, chunks=(block_size, block_size))
        if not isinstance(Z, da.Array):
        ...
        if not isinstance(B, da.Array):
            B = da.from_array(B, chunks=(block_size, _P))
        mu = da.exp(kronecker_vector_product(Z, B, beta, use_dask=True, block_size=block_size))
        nll = -log_nb_likelihood(mu, Y, alpha, use_dask=True, block_size=block_size)
        nll = nll.compute()
    return nll
...
def fit_multiplicative_log_glm(Z, B, Y, marginal_dist,
                               tol=1e-10, max_iter=100, lr=1.0,
    ...
    for iteration in range(max_iter):
        if marginal_dist == "Poisson":
            G = compute_gradient(Z, B, beta, Y, marginal_dist=marginal_dist, mode=gradient_mode, block_size=block_size)
        elif marginal_dist == "NB":
            G = compute_gradient(Z, B, beta, Y, marginal_dist=marginal_dist, alpha=alpha, mode=gradient_mode, block_size=block_size)
        # shared preconditioner for all marginal distributions
        C = compute_preconditioner(Z, B, beta, mu_Z=mu_Z, mu_X=mu_B, 
                                   mode=preconditioner_mode, block_size=block_size, damping_factor=1e-4)
        beta_new = beta - lr * C @ G
            # alpha = np.sum((Y-mu_tmp)**2 - mu_tmp) / np.sum(mu_tmp**2)
        # if iteration % 5 == 0:
        #     np.savez(f"{checkpoint_path}/iter_{iteration}.npy", beta=beta_new, G=G, C=C)
        delta_beta = np.linalg.norm(beta_new - beta)
        beta = beta_new
        if marginal_dist == "NB":
            mu_tmp = compute_mu(Z, B, beta, mode=gradient_mode, block_size=block_size).reshape((_M, _N))
            gamma = np.log(np.maximum(1e-8, np.sum((Y-mu_tmp)**2 - mu_tmp) / np.sum(mu_tmp**2)))
            alpha = np.exp(gamma)
            print(alpha, "alpha")
        if compute_nll:
            if marginal_dist == "Poisson":
                ...
                nll = compute_log_nb_nll(Z, B, beta, Y, alpha, mode=nll_mode, block_size=block_size)
            logging.info(f"--> Iteration: {iteration}, delta beta: {delta_beta}, NLL: {nll}")
